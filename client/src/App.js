...
const Btn = ({ children, onClick, variant = "primary", small, disabled, style: sx }) => {
  ...
  const variants = {
    primary: { background: "#c9a84c", color: "#1a1714" },
    secondary: { background: "#2a2723", color: "#c9a84c", border: "1px solid #3d3930" },
    danger: { background: "#5c1a1a", color: "#f87171" },
    ghost: { background: "transparent", color: "#a09882" },
    success: { background: "#1a3a2a", color: "#4ade80" },
  };
...
const Table = ({ columns, data, onRowClick }) => (
  <div style={{ overflowX: "auto" }}>
    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
      <thead>
        <tr>{columns.map((c) => <th key={c.key} style={{ textAlign: "left", padding: "10px 12px", color: "#8a8070", fontWeight: 600, fontSize: 11, textTransform: "uppercase", letterSpacing: 0.8, borderBottom: "1px solid #2d2a24" }}>{c.label}</th>)}</tr>
      </thead>
      <tbody>
        {data.length === 0 && <tr><td colSpan={columns.length} style={{ padding: 30, textAlign: "center", color: "#5a5040" }}>No records found</td></tr>}
        {data.map((row, i) => (
          <tr key={row.id || row.room_id || row.guest_id || row.reservation_id || row.payment_id || i} onClick={() => onRowClick?.(row)} style={{ cursor: onRowClick ? "pointer" : "default", borderBottom: "1px solid #1e1b17" }}>
...
export default function App() {
  ...
  const createGuest = async (data) => {
    const result = await api("/guests", { method: "POST", body: data });
    await reload();
    return result;
  };
  const createRoom = async (data) => {
    await api("/rooms", { method: "POST", body: data });
    await reload();
  };
  const createRoomType = async (data) => {
    await api("/room-types", { method: "POST", body: data });
    await reload();
  };
  ...
  const openNewReservation = (prefillRoomId) => {
    setModal({
      ...
      form: { guestMode: "existing", guestId: guests[0]?.guest_id || "", firstName: "", lastName: "", email: "", phone: "", address: "", roomId: prefillRoomId || rooms[0]?.room_id || "", checkIn: today(), checkOut: "", depositAmount: "", depositMethod: "cash" },
    });
  };
  ...
  if (loading) {
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", background: "#141210", color: "#c9a84c", fontSize: 18, fontFamily: "'Libre Baskerville', serif" }}>
        Loading...
      </div>
    );
  }
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: "#141210", color: "#d4c9b4", minHeight: "100vh", display: "flex" }}>
      ...
      <nav style={{ width: 220, background: "#18150f", borderRight: "1px solid #2d2a24", padding: "24px 0", display: "flex", flexDirection: "column", flexShrink: 0, position: "sticky", top: 0, height: "100vh" }}>
        <div style={{ padding: "0 20px 28px", borderBottom: "1px solid #2d2a24" }}>
          ...
          <div style={{ fontSize: 10, color: "#8a8070", letterSpacing: 3, textTransform: "uppercase", marginTop: 2 }}>Reservation System</div>
        </div>
        <div style={{ marginTop: 16, flex: 1 }}>
          {navItems.map((n) => (
            <button key={n.key} onClick={() => { setTab(n.key); setSearch(""); setStatusFilter(""); }}
              style={{ display: "flex", alignItems: "center", gap: 10, width: "100%", padding: "11px 20px", border: "none", background: tab === n.key ? "#1e1b17" : "transparent", color: tab === n.key ? "#c9a84c" : "#8a8070", fontSize: 13, fontFamily: "inherit", fontWeight: tab === n.key ? 700 : 500, cursor: "pointer", borderLeft: tab === n.key ? "3px solid #c9a84c" : "3px solid transparent", transition: "all .15s" }}>
              <span style={{ fontSize: 16, width: 20, textAlign: "center" }}>{n.icon}</span>{n.label}
            </button>
          ))}
        </div>
        <div style={{ padding: "16px 20px", borderTop: "1px solid #2d2a24" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              <Card>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8 }}>
                  {rooms.map((r) => {
                    const colors = { available: "#4ade80", occupied: "#60a5fa", maintenance: "#fbbf24" };
                    return (
                ...
                {(dashboard.recentReservations || []).map((r) => (
                  <div key={r.reservation_id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: "1px solid #2d2a24" }}>
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: "#8a8070" }}>Room {r.room_number} · {r.check_in_date}</div>
                    </div>
            ...
            <Card>
              <Table columns={[
                ...
                { key: "status", label: "Status", render: (r) => {
                  const colors = { available: "#4ade80", occupied: "#60a5fa", maintenance: "#fbbf24" };
                  return <span style={{ color: colors[r.status] || "#888", fontWeight: 600, fontSize: 12, textTransform: "uppercase" }}>{r.status}</span>;
                }},
                { key: "actions", label: "", render: (r) => r.status === "available" ? <Btn small variant="secondary" onClick={(e) => { e.stopPropagation(); openNewReservation(r.room_id); }}>Book</Btn> : null },
              ]} data={rooms} />
            </Card>
          </>
        )}
          ...
          <>
            ...
            <Card>
              <Table columns={[
                ...
                return `${g.first_name} ${g.last_name}`.toLowerCase().includes(s) || (g.email || "").toLowerCase().includes(s);
              })} />
            </Card>
          </>
        )}
        ...
        {tab === "reservations" && (
          <>
            ...
            <Card>
              ...
              ]} data={reservations.filter((r) => !statusFilter || r.status === statusFilter)} onRowClick={openResDetail} />
            </Card>
          </>
        )}
        ...
        {tab === "room-types" && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 14 }}>
              {roomTypes.map((t) => {
                const count = rooms.filter((r) => r.type_id === t.type_id).length;
                return (
                  <Card key={t.type_id} style={{ borderTop: "3px solid #c9a84c" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                      <div>
                        <div style={{ fontSize: 16, fontWeight: 700, color: "#e8dcc8" }}>{t.type_name}</div>
                        <div style={{ fontSize: 12, color: "#8a8070", marginTop: 4 }}>{t.description}</div>
                      </div>
                      <div style={{ fontSize: 11, color: "#8a8070", background: "#141210", padding: "3px 8px", borderRadius: 4 }}>{count} rooms</div>
                    </div>
                    <div style={{ marginTop: 14, fontSize: 22, fontWeight: 800, color: "#c9a84c" }}>{fmt(t.nightly_rate)}<span style={{ fontSize: 11, fontWeight: 400, color: "#8a8070" }}> /night</span></div>
                  </Card>
                );
              })}
            </div>
        ...
        <Modal title="New Reservation" onClose={() => setModal(null)} wide>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginTop: 14 }}>
            <Select label="Room" value={modal.form.roomId} onChange={(e) => setModal({ ...modal, form: { ...modal.form, roomId: Number(e.target.value) } })}>
              {rooms.map((r) => (
                <option key={r.room_id} value={r.room_id} disabled={r.status !== "available"}>
                  Room {r.room_number} — {r.type_name} {r.status !== "available" ? `(${r.status})` : ""}
                </option>
              ))}
            </Select>
          ...
          {modal.form.checkIn && modal.form.checkOut && modal.form.roomId && (() => {
            ...
            return (
              <div style={{ marginTop: 14, padding: 12, background: "#141210", borderRadius: 8, fontSize: 13 }}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 20 }}>
            ...
            <Btn onClick={async () => {
              ...
              if (modal.form.guestMode === "new") {
                if (!modal.form.firstName || !modal.form.lastName) return alert("Guest name required");
                const ng = await createGuest({ first_name: modal.form.firstName, last_name: modal.form.lastName, email: modal.form.email || null, phone: modal.form.phone || null, address: modal.form.address || null });
                guestId = ng.guest_id;
              }
              ...
              const res = await api("/reservations", {
                method: "POST",
                body: { guest_id: Number(guestId), room_id: Number(modal.form.roomId), check_in_date: modal.form.checkIn, check_out_date: modal.form.checkOut, total_amount: total },
              });
              if (modal.form.depositAmount && Number(modal.form.depositAmount) > 0) {
                await createPayment({ reservation_id: res.reservation_id, amount: Number(modal.form.depositAmount), payment_type: "deposit", payment_method: modal.form.depositMethod, payment_date: today() });
              }
              await reload();
              setModal(null);
              setTab("reservations");
        ...
        <Modal title="Check Out" onClose={() => setModal(null)}>
          ...
          {modal.balance > 0 && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 14 }}>
              ...
              <Select label="Method" value={modal.payMethod} onChange={(e) => setModal({ ...modal, payMethod: e.target.value })}>
                <option value="cash">Cash</option><option value="card">Card</option><option value="bank_transfer">Bank Transfer</option><option value="gcash">GCash</option>
              </Select>
            </div>
          )}
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 20 }}>
            <Btn variant="secondary" onClick={() => setModal(null)}>Cancel</Btn>
            <Btn onClick={async () => {
              if (modal.balance > 0 && Number(modal.payAmount) > 0) {
                await createPayment({ reservation_id: modal.res.reservation_id, amount: Number(modal.payAmount), payment_type: "full", payment_method: modal.payMethod, payment_date: today() });
              }
              await updateResStatus(modal.res.reservation_id, "checked_out");
              setModal(null);
      ...
      {modal?.type === "add-payment" && (
        <Modal title={`Add Payment — Reservation #${modal.res.reservation_id}`} onClose={() => setModal(null)}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            ...
            <Select label="Method" value={modal.method} onChange={(e) => setModal({ ...modal, method: e.target.value })}>
              <option value="cash">Cash</option><option value="card">Card</option><option value="bank_transfer">Bank Transfer</option><option value="gcash">GCash</option>
            </Select>
          </div>
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 20 }}>
            <Btn variant="secondary" onClick={() => setModal(null)}>Cancel</Btn>
            <Btn onClick={async () => {
              if (!modal.amount || Number(modal.amount) <= 0) return alert("Enter a valid amount");
              await createPayment({ reservation_id: modal.res.reservation_id, amount: Number(modal.amount), payment_type: "partial", payment_method: modal.method, payment_date: today() });
              setModal(null);
            }}>Record Payment</Btn>
          </div>
        </Modal>
      ...
      {modal?.type === "new-room" && (
        <Modal title="Add Room" onClose={() => setModal(null)}>
          <div style={{ display: "grid", gap: 10 }}>
            ...
            <Select label="Room Type" value={modal.form.typeId} onChange={(e) => setModal({ ...modal, form: { ...modal.form, typeId: Number(e.target.value) } })}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 20 }}>
            <Btn variant="secondary" onClick={() => setModal(null)}>Cancel</Btn>
            <Btn onClick={async () => { if (!modal.form.number) return alert("Room number required"); await createRoom({ room_number: modal.form.number, type_id: modal.form.typeId, floor: modal.form.floor }); setModal(null); }}>Add Room</Btn>
          </div>
        </Modal>
      )}
      ...
      {modal?.type === "new-room-type" && (
        <Modal title="Add Room Type" onClose={() => setModal(null)}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 20 }}>
            <Btn variant="secondary" onClick={() => setModal(null)}>Cancel</Btn>
            <Btn onClick={async () => { if (!modal.form.name || !modal.form.rate) return alert("Name and rate required"); await createRoomType({ type_name: modal.form.name, description: modal.form.description, nightly_rate: Number(modal.form.rate) }); setModal(null); }}>Add Type</Btn>
          </div>
        </Modal>
      )}
      ...
      {modal?.type === "res-detail" && (
        <Modal title={`Reservation #${modal.res.reservation_id}`} onClose={() => setModal(null)} wide>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div>
              <div style={{ fontSize: 11, color: "#8a8070", textTransform: "uppercase", fontWeight: 600, marginBottom: 6 }}>Guest</div>
              <div style={{ fontSize: 15, fontWeight: 700 }}>{modal.res.first_name} {modal.res.last_name}</div>
              <div style={{ fontSize: 12, color: "#8a8070", marginTop: 2 }}>{modal.res.guest_email} · {modal.res.guest_phone}</div>
            </div>
            <div>
              <div style={{ fontSize: 11, color: "#8a8070", textTransform: "uppercase", fontWeight: 600, marginBottom: 6 }}>Room</div>
              <div style={{ fontSize: 15, fontWeight: 700 }}>Room {modal.res.room_number} — {modal.res.type_name}</div>
              <div style={{ fontSize: 12, color: "#8a8070", marginTop: 2 }}>{fmt(modal.res.nightly_rate)} / night</div>
            </div>
            <div>
              <div style={{ fontSize: 11, color: "#8a8070", textTransform: "uppercase", fontWeight: 600, marginBottom: 6 }}>Stay</div>
              <div style={{ fontSize: 13 }}>{modal.res.check_in_date} → {modal.res.check_out_date}</div>
              <div style={{ fontSize: 12, color: "#8a8070" }}>{modal.nights} night{modal.nights > 1 ? "s" : ""}</div>
            </div>
            <div>
              <div style={{ fontSize: 11, color: "#8a8070", textTransform: "uppercase", fontWeight: 600, marginBottom: 6 }}>Status</div>
              <Badge status={modal.res.status} />
            </div>
          </div>
          <div style={{ marginTop: 20, padding: 14, background: "#141210", borderRadius: 8 }}>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, marginBottom: 6 }}>
              <span style={{ color: "#8a8070" }}>Total</span><span style={{ fontWeight: 700 }}>{fmt(modal.total)}</span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, marginBottom: 6 }}>
              <span style={{ color: "#8a8070" }}>Paid</span><span style={{ fontWeight: 700, color: "#4ade80" }}>{fmt(modal.paid)}</span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14, borderTop: "1px solid #2d2a24", paddingTop: 8, marginTop: 4 }}>
              <span style={{ fontWeight: 700 }}>Balance</span><span style={{ fontWeight: 800, color: modal.total - modal.paid > 0 ? "#f87171" : "#4ade80" }}>{fmt(Math.max(0, modal.total - modal.paid))}</span>
            </div>
          </div>
